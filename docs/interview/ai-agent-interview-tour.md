...
## 1 从项目讲起：先别急着报框架名 { #project-storytelling }
...
## 2 LangGraph：checkpoint 不是「存档」 { #langgraph }
...
## 3 上下文与记忆：难的不是「记住」，是「记对」和「别记爆」 { #memory }
...
## 4 Agent 形态：ReAct 与 Plan-Execute 差的不只是「先想再做」 { #agent-shapes }
...
## 5 工具 / Skill / MCP：「能力」还是「契约」 { #tools-skills-mcp }
